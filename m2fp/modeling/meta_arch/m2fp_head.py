...
class M2FPHead(nn.Module):
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        if cfg.MODEL.M2FP.TRANSFORMER_IN_FEATURE == "transformer_encoder":
            ...
            transformer_predictor_in_channels = input_shape[
                cfg.MODEL.M2FP.TRANSFORMER_IN_FEATURE
            ].channels
    ...
    def layers(self, features, mask=None):
        ...
        if self.ohmrm_enabled and "pred_masks" in predictions:
            ohmrm_out = self.ohmrm(
                mask_features,
                predictions["pred_masks"],
                multi_scale_features=multi_scale_features,
            )

            predictions["pred_masks"] = ohmrm_out["pred_masks_refined"]
            predictions["ohmrm_boundary_logits"] = ohmrm_out["boundary_logits"]
            predictions["ohmrm_occlusion_logits"] = ohmrm_out["occlusion_logits"]
